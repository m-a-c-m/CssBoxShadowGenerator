...
import Tool from "@/components/CssBoxShadowGenerator";
      ...
      <main className="min-h-screen px-4 py-12">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-center">
            ...
            <h1 className="mb-3 text-4xl font-bold text-white md:text-5xl">CSS Box Shadow Generator</h1>
            <p className="mb-2 text-lg text-text-muted">Create CSS shadows with multiple layers. Offset, blur, spread, color, opacity and inset per layer.</p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
            { icon: "🥞", title: "Layered shadows", desc: "Stack multiple shadow layers with offset, blur, spread and inset." },
            { icon: "🎨", title: "Color control", desc: "Pick color and opacity per layer with a live preview." },
            { icon: "🔒", title: "100% private", desc: "CSS is generated locally, ready to paste." },
            ].map((item) => (
          ...
          <div className="mt-8 rounded-xl border border-border/20 bg-white/3 p-6">
            ...
            <p className="mb-4 text-sm text-text-muted">Add CSS Box Shadow Generator to any page with a simple iframe, or link to it with attribution.</p>
            <div className="mb-3 rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Iframe (plug & play):</p>
              <code className="text-xs text-green-400 break-all">{`<iframe src="${EMBED_URL}" width="100%" height="700" style="border:none;border-radius:12px;" title="CSS Box Shadow Generator — miguelacm.es" loading="lazy"></iframe>`}</code>
            </div>
            <div className="rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Link with attribution (recommended for backlink):</p>
              <code className="text-xs text-green-400 break-all">{`<a href="${SITE_URL}" target="_blank" rel="noopener">CSS Box Shadow Generator — free tool by MACM</a>`}</code>
            </div>
          </div>
        </div>
      </main>
